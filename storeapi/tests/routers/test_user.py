...
async def test_register_user(async_client: AsyncClient):
    response = await register_user(async_client, "test@example.net", "123")
...
async def test_register_user_already_exists(async_client: AsyncClient, registered_user: dict):
    response = await registered_user(async_client, registered_user["email"], registered_user["password"])
