...
async def test_create_post(async_client: AsyncClient, registered_user: dict, logged_in_token: str):
    ...
    response = await async_client.post("/post",json={"body": body}, headers={"Authorazation": f"Bearer {logged_in_token}"})
...
async def test_get_comments_on_post(
    ...
    assert response.status_code == 200
    assert response.json() == [create_comment]
